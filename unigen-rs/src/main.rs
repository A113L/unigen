...
// Suppress the console window that Windows otherwise opens alongside the
// GUI window for any binary linked with the default (console) subsystem.
// Only applied to release builds (`not(debug_assertions)`) so `cargo run`
// during development still gets a console to show `env_logger`/println
// output and panic backtraces; a `cargo build --release` binary opens with
// just the egui window, no black terminal alongside it. No effect on
// Linux/macOS, which never had this console-window behavior to begin with.
...
fn main() -> eframe::Result<()> {
    ...
    let options = eframe::NativeOptions {
        ...
        depth_buffer: 0,
        stencil_buffer: 0,
        // vsync=false was originally disabled unconditionally to dodge
        // presentation-timing quirks on virtualized display adapters
        // (VirtualBox Mesa/SVGA3D) that mishandle swap-interval. On real
        // Windows 11 GPU drivers, however, disabling vsync with the glow/
        // WGL backend has the opposite effect: the compositor (DWM) only
        // flips the swapchain into the visible surface on its own
        // schedule, and without vsync forcing a synced present each
        // frame, the window can sit showing an unpresented (black)
        // backbuffer until *something else* forces DWM to redraw it —
        // e.g. dragging the window, which is exactly the "black window
        // until I move it" symptom. Keeping vsync on for Windows forces a
        // proper synced present every frame and avoids this; the
        // VM-driver workaround is kept for other platforms where it was
        // actually needed.
